...
Deze functie kunnen we vervolgens naar believen \textsl{aanroepen} om de code van de functie uit te voeren.
...
In listing~\ref{cod:slaregelsover3} zien we hoe een functiedeclaratie kan worden gebruikt\footnote{%
...
\booklisting[]{C}{Een programma waar op twee plaatsen een aantal regels wordt overgeslagen}{slaregelsover5}{c}{!ht}
...
\begin{dosbox}[title=Uitvoer van het programma \texttt{print\_rechthoek},label=fig:print_rechthoek]
++++++++++++++++++++
+                  +
+                  +
+                  +
...
Hoe dat opslaan van die terugkeeradressen wordt gerealiseerd is te zien in het kader op pagina~\pageref{fig:stack}.
...
\hspace*{1em}\texttt{printf("\%f"), sin(arg)); \ \ /* sin(arg) is printed */}
...
Vanuit de functie kan een waarde van het returntype worden teruggegeven met behulp van de \texttt{return}-statement\indexkeyword{return}.
...
In listing~\ref{cod:lees_geheel_getal} is een functie \texttt{lees\_geheel\_getal} gegeven die een geheel getal inleest, controleert of de ingevoerde waarde een getal is en of deze waarde tussen een als argument meegegeven minimale en maximale waarde ligt (inclusief).
...
\begin{dosbox}[title={Invoer van het programma},label=fig:lees_geheel_getal]
...
C is een blokgestructureerde taal dat betekent dat we in een \textsl{blok}\index{blokstructuur} lokale variabelen kunnen de declareren. Zie listing~\ref{cod:funvarbinnenblok}. Variabele \texttt{i} is alleen te gebruiken binnen het \texttt{if}-statement.
...
Als we een globale variabele alleen maar zichtbaar willen maken binnen het C-bestand waarin de variabele wordt gedeclareerd dan moeten we de variabele voorzien van de qualifier \texttt{static}\indextwo{static}{globale variabele}.
...
Als we deze recursieve definitie coderen in C krijgen we een recursieve functie zoals te zien is in listing~\ref{cod:faculteit_recursief}. Dit programma drukt de faculteiten af van $0$ t/m $20$.
...
Als we de functie binnenkomen, dan controleren we eerst of het meegegeven argument groter is dan 9. Zo ja, dan delen we het getal door 10 (daarmee ``verdwijnt'' de eenheid) en geven dit als argument mee aan weer een aanroep van \texttt{printdec}. Als het argument kleiner is dan 10, dan hebben we een eenheid en drukken we het argument af en keren terug naar de vorige aanroep. In de vorige aanroep drukken we de eenheid van het argument af.
...
Een pointer is een variabele met als inhoud het \textsl{adres} van een (andere) variabelen. Met behulp van pointers kunnen we dus niet een kopie van een variabele als argument meegeven, maar een adres waarop de (originele) variabele ter vinden is. Ook een functie, of eigenlijk, het beginadres van een functie kan als argument en parameter dienen. We bespreken dit in hoofdstuk~\ref{cha:pointers}.
...
Hoewel C niet specifiek bedoeld is voor het berekenen van wiskundige formules, kan met behulp van \textsl{mathematical library} toch wiskundige berekeningen gedaan worden. De functies zijn te zien in de onderstaande lijst.
...
\texttt{ceil(x)} & kleinste geheel getal niet kleiner dan $x$\\
\texttt{floor(x)} & grootste geheel getal niet groter dan $x$\\
