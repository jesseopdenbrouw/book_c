...
\draw [ghostarrow] (stop) -- (stop2);
\end{tikzpicture}
\caption{Flowchart van een if-else-statement.}
\end{figure}
...
wordt weergegeven als flowchart (figuur~\ref{fig:flowhile}):
...
Let erop dat \textsl{statements\textsubscript{2}} n\'a \textsl{statements\textsubscript{3}} komt.
De flowchart is hieronder te vinden.

\begin{figure}[H]
\centering
\begin{tikzpicture}
\node [ghostnode] (start) {};
\node [process, below =of start] (pro1) {\textsl{statements\textsubscript{1}}};
\node [ghostnode, below =of pro1] (start1) {};
\node (dec1) [decision, below =of start1] {\textsl{expressie}};
\node (pro2) [process, below =of dec1] {\textsl{statements\textsubscript{3}}};
\node (pro3) [process, below =of pro2] {\textsl{statements\textsubscript{2}}};
...
\draw [ghostarrow] (stop2) -- (stop3);
\end{tikzpicture}
\caption{Flowchart van een for-lus.}
\end{figure}
...
switch ((*\normalfont\textsl{variabele}*)) {
    ...
    ...
    case (*\normalfont\textsl{expressie\textsubscript{n}}*): (*\normalfont\textsl{statements\textsubscript{n}}*)
    default : (*\normalfont\textsl{statements\textsubscript{d}}*)
}
\end{lstlisting}
...
%%%\draw [ghostarrow] (stop) -- (stop2);
\end{tikzpicture}
\caption{Flowchart van een switch-statement.}
\label{fig:switch}
\end{figure}

\section{Voorbeeld}
We zullen een flowchart presenteren van een kort programma.
Hieronder is een een eenvoudig C-programma gegeven dat een variabele $k$ inleest en
vervolgens de som bepaalt van alle cijfer tussen 1 en $k$ (inclusief), dus:
%
\begin{equation}
j = 1 + 2 +3 + 4 +\cdots + k
\end{equation}
...
aangepast. Uiteindelijk komen uit bij het einde van het programma.
