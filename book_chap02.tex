...
Niet alle karakters kunnen zo worden ingevoerd. Een voorbeeld is het karakter \lstc{'} zelf (apostrofe of \textsl{quote} genoemd). Om dit karakter in te voeren gebruiken we een \textsl{escape sequence}\index{escape sequence}. Een escape sequence bestaat uit een \textsl{backslash}\index{backslash karakter} (\lstinline[style=lstoneline]|\|) en een karakter. We kunnen de apostrofe dus invoeren als \lstinline[style=lstoneline]|\'|. Om de backslash zelf in te voeren gebruiken we \lstinline[style=lstoneline]|\\|.
